...
export class LockUnlockComponent implements OnInit {
  ...
  onConfirm(): void {
    ...
    const desiredState =
      this.form.value.lockState === 'Locked' ? 'lock' : 'unlock';
    ...
    this.spinner
      .with(
        this.accessService.updateAction(
          `api/locks`,
          this.lock.lockId,
          desiredState,
          request
      ...
      .subscribe({
        next: (data) => {
          if (data.isLocked)
            this.accessService.createDoorLockedNotification(
              `${this.lock.displayName} locked successfully!`
            );
          else
            this.accessService.createDoorUnlockedNotification(
              `${this.lock.displayName} unlocked successfully!`
            );
